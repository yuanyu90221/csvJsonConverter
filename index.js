...
let csvfile = "mnemonic.csv";
let outputFileName = "mnemonic.json";
...
newConverter.on("end_parsed", function(jsonArray){
    ...
    //     });
    // });
    let resultArray = jsonArray.map((element)=>{
        let {number, words} = element;
        let jsonObj = {};
        jsonObj[`${number}`] = words;
        return jsonObj;
    });
    fs.writeFile(path.join("./result",outputFileName),JSON.stringify(resultArray),'utf8');
